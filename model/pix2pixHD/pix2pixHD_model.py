...
class Pix2PixHDModel(BaseModel):
    ...
    def forward(self, heatmaps, image, infer=False):
        ...
        pred_fake_pool = self.discriminate(heatmaps, fake_image, use_pool=True)
        loss_D_fake = self.criterionGAN(pred_fake_pool, False)        

        # Real Detection and Loss        
        pred_real = self.discriminate(heatmaps, real_image)
        loss_D_real = self.criterionGAN(pred_real, True)

        # GAN loss (Fake Passability Loss)        
        pred_fake = self.netD.forward(torch.cat((heatmaps, fake_image), dim=1))
    ...
    def forward_attention(self, heatmaps, image, atten_mask,infer=False):
        # Encode Inputs
        # input_label, inst_map, real_image, feat_map = self.encode_input(label, inst, image, feat)
        real_image = image
        ...
        # Fake Generation
        fake_image = self.netG.forward(heatmaps)
        fake_image = fake_image*atten_mask
    ...
    #def clamp_G_weights(self):
    #    for name, param in self.netG.named_parameters():
    #        if 'bias' not in name:
    #            param.data = torch.clamp(param.data, -0.9, 0.9)
