"""
Training script for Extended Hybrid Representation with Full 10-dimensional SMPL β Parameters
...
from Datasets.upperbody_garment.upperbody_garment_beta10 import UpperBodyGarmentBeta10, UpperBodyGarmentBeta10Hybrid
...
def main():
    ...
    # Override for Full 10-dimensional β Hybrid Representation
    # I_vm (3) + I_sdp (3) + I_β (10) = 16 channels
    opt.input_nc = 16
    opt.output_nc = 4  # RGB + Alpha
    opt.model = 'pix2pixHD_RGBA'  # Use RGBA model for 4-channel output
    
    print("=" * 60)
    print("Full 10-dimensional β Hybrid Representation Training")
    print(f"Input channels: {opt.input_nc} (I_vm: 3, I_sdp: 3, I_β: 10)")
    ...
    if opt.dataset_path is not None:
        ...
        # Use the new dataset with full 10-dimensional β parameters
        dataset = UpperBodyGarmentBeta10(path_list[0], img_size=opt.img_size, use_random_beta=False)
        
        if len(path_list) > 1:
            for i in range(1, len(path_list)):
                dataset = dataset + UpperBodyGarmentBeta10(path_list[i], img_size=opt.img_size, use_random_beta=False)
    else:
    ...
    # Create model with extended input channels (16)
    ...
    for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(dataloader):
            # Unpack data: garment_img, vm_img, dp_img, beta_img (10ch), garment_mask
            ...
            # Create Full 10-dimensional β Hybrid Representation
            ...
            if save_fake:
                ...
                # Visualize β[0] map (first channel of 10D β, repeat to RGB for visualization)
